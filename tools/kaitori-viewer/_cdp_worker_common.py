...
def run_worker(worker_fn, extra_headers=None):
    """CDPワーカーのエントリポイント。

    1. CDP接続を試行（Brave/Chrome）
    2. 失敗時はPlaywright直接起動にフォールバック
    3. worker_fn(context, jan_code) を呼び出し
    4. 結果をJSON出力
    ...
    try:
        ...
        from stealth import create_cdp_stealth_context, create_stealth_context
