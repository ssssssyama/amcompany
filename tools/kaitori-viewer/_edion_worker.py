"""エディオン検索ワーカー（サブプロセスとして実行される）

検索URL: https://www.edion.com/item_list.html?keyword={JAN}
ブラウザ: CDP接続（実Chrome）

エディオンはPlaywright headlessでブロックされるため、CDP接続モードで実行する。
"""
import random
import re
import sys
...
def _verify_product_page(context, candidate, jan_code):
    ...
    try:
        ...
        title_el = page.query_selector("h1, .productPrice, .productName, .product-name")
        ...
        page_price = None
        for sel in [".productPrice", ".itemPrice .num", ".price", "[class*='price']"]:
...
def search(context, jan_code):
    ...
    try:
        page.goto(
            f"https://www.edion.com/item_list.html?keyword={jan_code}",
            timeout=20000,
        )
        page.wait_for_load_state("domcontentloaded", timeout=10000)
        ...
        for item in items[:10]:
            ...
            name = ""
            for name_sel in ["a[href*='item']", "h3", "h2", ".productName", ".product-name"]:
            ...
            price = None
            for price_sel in [".productPrice", ".itemPrice .num", ".price", "[class*='price']"]:
            ...
            url = ""
            link_el = item.query_selector("a[href*='item']") or item.query_selector("a[href]")
            if link_el:
                href = link_el.get_attribute("href") or ""
                if href.startswith("/"):
                    url = f"https://www.edion.com{href}"
            ...
            if price:
                ...
                if name and not is_used_or_excluded(name) and page.url.startswith("http"):
                    candidates.append({
                        ...
                        "source": "エディオン", "stock_status": "unknown",
                    })
