"""パソコン工房検索ワーカー（PC専門、サブプロセスとして実行）

PCパーツ・自作PC関連に強い。JAN検索対応。
"""
import io
import json
import sys
...
def _find_first_card(page, jan: str):
    cards = page.query_selector_all(
        ".productList li, .product, .item, "
        "article, div[class*='product'], div[class*='item']"
    )
    ...
    for card in cards[:15]:
        ...
        if link:
            ...
            if href.startswith("/"):
                url = "https://www.pc-koubou.jp" + href
...
def main():
    ...
    url = f"https://www.pc-koubou.jp/products/list.php?currentPage=1&isSetParameter=&keyword={jan_code}"
    ...
    try:
        with sync_playwright() as p:
            ...
            ok = safe_goto(
                page, url,
                warmup_url="https://www.pc-koubou.jp/",
